  ...
  const unlockPDF = async () => {
    ...
    try {
      const response = await axios.post(
        `${process.env.REACT_APP_BACKEND_URL}/pdf/unlock`,
        formData,
        {
          responseType: "blob",
      ...
      alert("Failed to unlock. Wrong password or secured file.");
      console.error(error);
    }
  ...
  return (
    <div className="flex justify-center mt-12">
      <div className="bg-white shadow-xl p-10 rounded-xl w-full max-w-xl text-center">
        ...
        <h1 className="text-3xl font-bold text-blue-600">Unlock PDF</h1>
        <p className="text-gray-500 mt-1">Remove password from secured PDF file</p>

        {/* Upload */}
        <div className="border-2 border-dashed border-blue-400 rounded-lg p-8 mt-6 cursor-pointer hover:bg-blue-50">
          <input type="file" id="unlockPDF" className="hidden" accept=".pdf"
            onChange={handleFileUpload}
          />
          <label htmlFor="unlockPDF" className="cursor-pointer">
